...
const CaseView = () => {
  ...
  const handleSlideChangeMobile = (swiper: any) => {
    ...
    if (
      ...
      textRefMob.current.style.transition =
    ...
    if (scrollRef ?? scrollRef2) {
      ...
      setPositionCursor(
        current
          ? (current?.offsetHeight / (projects.length - 1)) * currentIndex - 22
          : -22,
      );
    }
  };
            ...
            {projects.map((item, index) => {
              return (
                <div
                  ...
                  className="z-10 ml-24 mt-9 inline-flex flex-col items-start gap-3"
                  key={index}
                  ref={
                    index == 0
                      ? text0Ref
      ...
      <div className="relative flex h-full w-full flex-row justify-center">
        <div className="mb-8 ml-10 mt-10 flex max-w-[360px] flex-col lg:hidden">
          <div className="relative flex h-[260px] w-full flex-row">
            ...
            <Link
              href={project.link}
              className="pointer-events-auto absolute relative z-10 mr-8 h-full w-full cursor-pointer">
              <Swiper
